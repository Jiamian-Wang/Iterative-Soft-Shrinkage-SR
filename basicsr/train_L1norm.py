...
# @WJM: zero out pruned weights for unstructured pruning
...
def train_pipeline(root_path):
    ...
    assert args.prune_method in ['L1', 'GReg-1', 'ASSL'], 'ERROR: invalid args.prune_method!'
    class passer:pass
    ...
    # check_model(model.net_g_ema)
    pruner = pruner_dict[args.prune_method].Pruner(model=model.net_g, args=args, logger=logger, passer=passer)
    ...
    if args.prune_method and args.wg == 'weight':
        ...
        # model.net_g.load_state_dict(_model)
    # check_model(model.net_g)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if current_iter % args.grad_save_iters == 0:
                ...
                print('>>>@FT: model_grad_status shape, should be [#layer, 2(norm, var)]',
                      np.array(model_grad_status).shape)  # verified [103,2]
