...
from .utils import get_score_layer, pick_pruned_layer, get_masks
pjoin = os.path.join
...
class Pruner(MetaPruner):
    def __init__(self, model, opt, args, logger, passer):
        ...
        self.grad_prune_dict = passer.grad_prune_dict

        self.mask_prune_dict = {}

        self.param_prune_dict = {}
        ...
        if args.greg_mode in ['part'] and self.prune_state in ['update_reg']:
            self._get_kept_wg_L1(model=self.model.net_g, align_constrained=True) # this will update the 'self.kept_wg', 'self.pruned_wg', 'self.pr'
    ...
    def _iht(self):
        for name, m in self.model.net_g.named_modules():
            if name in self.layers:
                ...
                out = get_score_layer(m, wg=self.args.wg, criterion=self.args.prune_criterion)
                score = out['score']
                layer.score = score

                # local pruning
                if self.args.compare_mode in ['local']:
                    self.pruned_wg[name], self.kept_wg[name] = pick_pruned_layer(score, self.N2M_granularity, self.pr[name], sort_mode=self.args.pick_pruned)
                else:
                ...
                if self.args.prune_method in ['IHT']:
                    mask[self.pruned_wg[name]] = 0
                elif self.args.prune_method in ['IST']:
                    mask[self.pruned_wg[name]] = self.args.ist_ratio
                mask = mask.view(layer.shape)
                m.weight.data.mul_(mask)
                # if hasattr(m, 'bias'): m.bias.data.mul_(mask[:, 0, 0, 0])

                if self.total_iter % self.args.mask_save_iters == 0:
                    self.mask_prune_dict[name].append(mask.detach().cpu().numpy())

                if self.total_iter % self.args.param_save_iters == 0:
                    self.param_prune_dict[name].append(m.weight.data.detach().cpu().numpy())
    ...
    def _greg_1(self, m, name):
        ...
        # @WJM: reget the pruned and kept wg according to the current iteration's weight magnitude
        layer = self.layers[name]
        out = get_score_layer(m, wg=self.args.wg, criterion=self.args.prune_criterion)
        score = out['score']
        layer.score = score
        # local pruning
        if self.args.compare_mode in ['local']:
            self.pruned_wg[name], self.kept_wg[name] = pick_pruned_layer(score, self.N2M_granularity, self.pr[name],
    ...
    def prune(self):
        ...
        for name, m in self.model.net_g.named_modules():
            if name in self.layers:
                self.mask_prune_dict[name] = []

        for name, m in self.model.net_g.named_modules():
            if name in self.layers:
                self.param_prune_dict[name] = []
        ...
        while True:
            finish_prune = self.train(logger) # there will be a break condition to get out of the infinite loop
            if finish_prune:
                return copy.deepcopy(self.model.net_g)
            # self.test()

# ************************************************ The code below refers to BasicSR ************************************************ #
    def train(self, logger):
        ...
        while train_data is not None:
            ...
            if self.total_iter % self.args.print_interval == 0:
                ...
                # prune_state： SSA, method: ASSL, compare_mode: local, greg mode: part
            ...
            if self.args.apply_reg and self.args.prune_method in ['IST-GReg']:  # reg can also be not applied, as a baseline for comparison
                self._apply_reg()

            self.model.optimize_parameters_afterGRegfunc(loss_dict=loss_dict)

            if self.total_iter % self.args.grad_save_iters == 0:
                ...
                self.grad_prune_dict[self.total_iter] = np.array(model_grad_status)

                # save the current dict into the file for visualization
                if self.total_iter % self.args.grad_write2file_iters == 0:
                    np.save(osp.join(self.opt['path']['log'], 'grad_prune_dict_%d.npy'%self.total_iter), self.grad_prune_dict)


            # @WJM: do the _iht() after the optimzier.step(), the same as the ASSL/ist_pruner
            if self.args.prune_method in ['IHT', 'IST']:
                self._iht()

            if self.total_iter % self.args.mask_write2file_iters == 0:
                mask_save_pth = osp.join(self.opt['path']['log'],'mask_save')
                if not osp.exists(mask_save_pth):
                    os.makedirs(mask_save_pth)
                np.save(osp.join(mask_save_pth, 'mask_prune_dict_%d.npy' % self.total_iter),
                        self.mask_prune_dict)

            if self.total_iter % self.args.param_write2file_iters == 0:
                param_save_pth = osp.join(self.opt['path']['log'],'param_save')
                if not osp.exists(param_save_pth):
                    os.makedirs(param_save_pth)
                np.save(osp.join(param_save_pth, 'param_prune_dict_%d.npy' % self.total_iter),
                        self.param_prune_dict)
            ...
            if self.prune_state in ["stabilize_reg"] and self.total_iter - self.iter_stabilize_reg == self.args.stabilize_reg_interval:
                ...
                if self.args.greg_mode in ['all']:
                    self._get_kept_wg_L1(model=self.model.net_g, align_constrained=True)
