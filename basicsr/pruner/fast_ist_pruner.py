...
from .utils import get_score_layer, pick_pruned_layer, get_masks, fast_pick_pruned_layer
pjoin = os.path.join
...
class Pruner(MetaPruner):
        ...
        for name, m in self.model.net_g.named_modules():
            if name in self.layers:
                ...
                if self.args.compare_mode in ['local']:
                    self.pruned_wg[name], self.kept_wg[name] = fast_pick_pruned_layer(score, self.N2M_granularity, self.pr[name], sort_mode=self.args.pick_pruned)
                else:
    ...
    def _greg_1(self, m, name):
        ...
        # local pruning
        if self.args.compare_mode in ['local']:
            self.pruned_wg[name], self.kept_wg[name] = fast_pick_pruned_layer(score, self.N2M_granularity, self.pr[name],
